...
<p align="center">
  ...
  课表、成绩、二课、学工、一卡通、考级报名与桌面组件，UI 是一整套液态玻璃
</p>
...
五个页面装下全部高频操作：**课表 · 学工 · 成绩 · 二课 · 我的**。冷启动欢迎回来，登录一次自动续期，通知直达课程详情。

课表、考试、成绩、二课积分、未读消息还能各做成一张卡片摆进「组件工作台」，摆成什么顺序由你决定；同一批卡片可以直接钉到手机桌面上，锁屏解锁第一眼就看到。
...
<table>
  <tr>
    ...
    <td align="center" width="33%">
      <img src="pic/widget-board.jpg" width="100%"/><br/>
      <sub><b>组件工作台</b>：七种卡片随手摆，同一批还能钉到手机桌面</sub>
    </td>
    <td align="center" width="33%">
  ...
  <tr>
    ...
    <td align="center" width="33%">
      ...
      <sub><b>课表设置</b>：自定义课程、开学日期、节次时间、显示密度与周末开关</sub>
    </td>
  </tr>
</table>
...
- 课表周视图，课程自动配色，可导出 `.ics` 到系统日历；没有周末课时可以把周末两列收起来，五列比七列宽出约四成
- 日视图按天翻页，正在上的课与下一节标出来；顶上的日期条带一个跟着翻页走的滑块，今天那一格另有一个小圆点
- 课表字号四档可调（小 / 标准 / 大 / 特大）、显示密度标准 / 紧凑，只影响课表，其它页面不动
- 时间冲突的课，在课程详情里手动指定本节要上的那门，另一门自动让位隐藏（按节记录、按账号保存）
- 补课（调休）的「补到第几周 / 哪一天 / 要补哪一周 / 哪一天」四项都能自己选，落点不在当前周会自动翻过去
- 成绩按学期查，GPA 自动算，另外还有总体成绩和考试安排

组件工作台与桌面组件：

- 应用内一块能自己摆布的信息页，七种卡片：下一节课 / 今日课表 / 今日时间轴 / 考试倒计时 / 成绩概览 / 第二课堂积分 / 未读消息
- 四列网格、长按拖动换位、逐张改尺寸；组件抽屉里能先看**桌面上最小尺寸的样子**，再决定加不加
- 同一批卡片可以直接放到手机桌面（壁纸上面那一层），点一下跳到对应页面，课表类卡片自动回到本周今天
- 桌面组件读本地缓存，不打开应用也能看；登录状态、课表同步、消息变化与上下课时间都会自动刷新
- ColorOS（OPPO / 一加 / realme）上的添加路径：长按桌面空白处 →「卡片」→ 卡片中心 → 一直往下滑到最底部 →「插件」→ 选「校园助理」；其它系统也可以在工作台里直接点「添加到桌面」
...
- 每门课都能单独开上课提醒，通知直达那门课的详情；提醒基于系统精确闹钟，应用被杀也能响
- 总开关与提前时间集中在「课表设置 → 课程提醒」（上课时 / 提前 5、10、15、20、30、45、60 分钟，默认 15 分钟）
- **上课自动模式**：上课时自动切到勿扰 / 仅闹钟 / 仅优先，下课时还原成你原来的档位；没有勿扰权限时绝不去动系统设置（改了就可能还原不了）
- 页内自带权限自检：通知、精确闹钟、勿扰、后台自启、电池优化，每项都能直接点开对应的系统设置页

校园服务（「我的 → 校园服务」）：

- **一卡通**：校园卡余额、账户总额与宿舍电费只读查询；消费按月看支出与收入，逐笔明细一键跳学校官方账单页。登录走学校统一身份认证，**不保存你的一卡通密码，也不提供任何充值、缴费**。挂失 / 解挂是唯一的写操作，需要两段确认（手打一句确认语 + 查询密码），成功后状态以服务端为准
- **空闲教室**：按 校区 → 楼号 / 场地类别 / 座位数 / 名称关键词 与 周次 / 星期 / 节次 查没课的教室；楼号与节次的候选随校区自动切换，只读、不代提交场地借用申请
- **考级项目报名**：四六级、普通话等等级考试的报名与退报在应用内完成，复用教务登录态
...
- 底栏行为可调：向下滚动自动收起、向上展开，也可以关掉让它一直待在原地
...
| 选课 | **不提供**。1.2.6 起选课功能整体下线（入口、页面与相关代码一并移除），底栏第 2 格由「选课」改为「学工系统」；需要选课请使用学校教务网页 |
| 一卡通 | 只读查询（余额 / 消费汇总 / 宿舍电费）与挂失解挂；**不提供充值、缴费、转账** |

验证码、结果待确认或登录失效时需要人工处理；应用不会绕过学校限制。
...
Kotlin + Jetpack Compose（Material 3）。玻璃渲染用 Kyant Backdrop 2.0，`blur + lens + vibrancy` 三层，按设备能力分档，低端机降级到透镜采样。动效走 Compose Animation，spring/tween 曲线全收在 `MotionTokens` 里。网络 OkHttp + Coroutines，HTML 用 Jsoup 解析。桌面组件走标准 `AppWidgetProvider` + `RemoteViews`，渲染逻辑抽成纯函数以便单测；应用内的卡片与桌面卡片共用同一层数据装配，同一个数字在哪儿都一样。应用内更新由自带的下载器（OkHttp 流式下载 + FileProvider 唤起安装器）完成。安装包在本地构建后发布到 Gitee Release。
