...
object ScheduleIdentity {
    ...
    /**
     * 课程级配色键：同一门课在课表里占用的所有时段共用一种颜色。
     *
     * 配色**不能**拿 [network] 生成的 id 当键——那个 id 把星期、节次、周次都编了进去，
     * 于是"宏观经济学"排在周一 1-2 节和周三 3-4 节会得到两个 id、两种颜色。
     * 按课程名归并后，一门课走到哪一节都是同一个颜色。
     *
     * 归一化处理全角括号、大小写与多余空白，避免"大学英语（一）"和"大学英语(一)"被当成两门课。
     */
    fun colorKey(name: String): String =
        "course:" + Normalizer.normalize(name.trim(), Normalizer.Form.NFKC)
            .replace(Regex("\\s+"), " ")
