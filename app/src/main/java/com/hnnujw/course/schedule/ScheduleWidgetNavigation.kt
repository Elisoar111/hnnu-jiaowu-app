...
data class ScheduleWidgetRequest(
    val account: String,
    val school: String,
    val term: String,
    val course: String?,
...
/**
 * 桌面小组件的点击深链。组件点按 → 带 scheme 的 Intent 打开 MainActivity →
 * [accept] 解析成 [requested]（Compose state）→ ScheduleRoute 观察
 * `requested` 执行跳转（回今天 / 打开课程详情 / 登录 / 同步）。
 */
object ScheduleWidgetNavigation {
    const val ACTION = "com.hnnujw.course.action.OPEN_TODAY_SCHEDULE"
    ...
    internal fun intent(
        ...
        startsAt: Long? = null
    ): Intent = Intent(context, MainActivity::class.java).setAction(ACTION)
    ...
    fun accept(intent: Intent?) {
        ...
        requested = ScheduleWidgetRequest(
            data.getQueryParameter("account").orEmpty(), data.getQueryParameter("school").orEmpty(),
            data.getQueryParameter("term").orEmpty(), course, System.nanoTime(), action,
            data.getQueryParameter("startsAt")?.toLongOrNull())
    }
