...
/** 只读本地缓存 + 不精确的免唤醒边界闹钟；从不做任何网络认证。 */
object ScheduleWidgetUpdater {
    const val ACTION_REFRESH = "com.hnnujw.course.action.REFRESH_SCHEDULE_WIDGET"
    ...
    fun requestPin(context: Context, style: ScheduleWidgetStyle = ScheduleWidgetStyle.Double) {
        ...
        } else GlassToaster.show("长按桌面，在小组件中添加「${style.title}」")
    }

    /** 常驻监听：课表缓存 / 提醒日历 / 设置 / 登录态任一变化 → 去抖 100ms 后重画组件。 */
    ...
    /** 当前会话可展示的课表快照；未登录 / 演示模式 / 会话与账号不匹配 → null（组件显示"去登录"）。 */
    ...
    fun update(context: Context) {
        ...
        state.agenda?.let { agenda ->
            context.getSystemService(AlarmManager::class.java).set(
                AlarmManager.RTC, agenda.nextChangeAt.coerceAtLeast(now + 1_000), alarm(context))
        }
    }
