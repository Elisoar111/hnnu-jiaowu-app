package com.hnnujw.course.service
...
import com.hnnujw.course.utils.CourseNameKit
import com.hnnujw.course.utils.GrabTaskUtils.appendGrabLog
...
import com.hnnujw.course.academic.*
import com.hnnujw.course.utils.TeachingClassMatcher
...
class GrabService : Service() {
    ...
    companion object {
        ...
        // Action constants
        const val ACTION_START = "com.hnnujw.course.action.START_GRAB"
        const val ACTION_START_KEYWORD = "com.hnnujw.course.action.START_GRAB_KEYWORD"  // 关键词模式
        const val ACTION_START_QUEUE = "com.hnnujw.course.action.START_GRAB_QUEUE"      // 🔧 新增：直接队列模式
        const val ACTION_START_FUZZY_MATCH = "com.hnnujw.course.action.START_FUZZY_MATCH" // 🔧 模糊匹配模式
        const val ACTION_STOP = "com.hnnujw.course.action.STOP_GRAB"
        ...
        const val BROADCAST_UPDATE = "com.hnnujw.course.GRAB_UPDATE"
    ...
    private val cookieExpiredReceiver = object : android.content.BroadcastReceiver() {
        override fun onReceive(context: Context?, intent: Intent?) {
            if (intent?.action == com.hnnujw.course.network.CourseApiClient.ACTION_COOKIE_EXPIRED) {
                ...
                if (isRunning) {
                    Log.e(TAG, "收到服务账号 Cookie 失效广播，正在停止选课服务")
                    handleCookieInvalid()
                }
            }
        }
    ...
    override fun onCreate() {
        ...
        // 🔧 注册 Cookie 失效监听
        val filter = android.content.IntentFilter(com.hnnujw.course.network.CourseApiClient.ACTION_COOKIE_EXPIRED)
    ...
    private fun bindServiceAccount(intent: Intent?): Boolean {
        ...
        if (isRunning
            ...
            broadcastLogForAccount(requestAccountStorageKey, "已有其他账号的选课任务正在运行，请先停止后再启动")
            return false
        }
        ...
        if (serviceAccountKey.isNotBlank() && serviceAccountKey != userManager.currentAccountKey) {
            ...
            if (!switched) {
                broadcastLogForAccount(serviceAccountStorageKey, "选课启动失败：找不到绑定账号，请重新登录")
                return false
            }
        }
        ...
        if (currentSchool == null) {
            broadcastLogForAccount(serviceAccountStorageKey, "选课启动失败：当前账号未登录")
            return false
        }
        return true
    }
    ...
    override fun onStartCommand(intent: Intent?, flags: Int, startId: Int): Int {
        if (intent?.action == ACTION_STOP) {
            if (isStopRequestForServiceAccount(intent)) {
                ...
                val requestAccountStorageKey = intent.getStringExtra(EXTRA_ACCOUNT_STORAGE_KEY).orEmpty()
                broadcastLogForAccount(requestAccountStorageKey, "当前账号没有正在运行的选课任务")
            }
            return START_STICKY
        }
        ...
        when (intent?.action) {
            ACTION_START -> {
                ...
                if (targetCourse != null && currentSchool != null) {
                    startForeground(NOTIFICATION_ID, createNotification("正在选课: $courseName"))
                    startGrabbing()
                } else {
            ...
            ACTION_START_QUEUE -> {
                ...
                checkCookieValidity(currentSchool!!) { isValid ->
                    ...
                    val modeText = if (isParallelMode && queue.size > 1) "并行队列选课" else "直接队列选课"
                    startForeground(NOTIFICATION_ID, createNotification("正在准备队列选课..."))
                    ...
                    if (isParallelMode && queue.size > 1) {
                        startParallelCourseTasks(queue, origin = "queue", label = "队列选课")
                    } else {
                        startNextQueueItem()
                    }
                }
    ...
    private fun startAcademicQueue(intent: Intent) {
        ...
        if (items.isEmpty()) {
            startForeground(NOTIFICATION_ID, createNotification("选课队列为空"))
            broadcastLog("当前账号的选课队列为空")
            stopForeground(STOP_FOREGROUND_REMOVE)
            stopSelf()
            return
        }
        ...
        startForeground(NOTIFICATION_ID, createNotification("教务选课：${items.size} 门课程"))
        broadcastLog("开始教务选课：${items.size} 门课程，$workers 个执行任务")
                                ...
                                if (!active.sessionState.isCurrent(expected) || !SessionRenewer.canRenew()) false
                                    ...
                                    SessionRenewer.request(expected) { result ->
                                        if (continuation.isActive) {
                                            if (result is com.hnnujw.course.utils.SessionRecoveryResult.Recovered && active.sessionState.isCurrent(result.token)) {
                                            ...
                                            } else if (result is com.hnnujw.course.utils.SessionRecoveryResult.Superseded) {
                                                continuation.cancel()
    ...
    private fun postAcademicAttention(courseName: String, message: String) {
        ...
        val notification = NotificationCompat.Builder(this, CHANNEL_ID)
            .setSmallIcon(R.mipmap.ic_launcher).setContentTitle("选课任务需要处理")
    ...
    private fun createNotification(content: String): Notification {
        ...
        return NotificationCompat.Builder(this, CHANNEL_ID)
            .setContentTitle("选课进行中")
            .setContentText(content)
            .setSmallIcon(R.mipmap.ic_launcher)
            .setOngoing(true)
            .setContentIntent(pendingIntent)
            .addAction(0, "停止选课", stopPendingIntent)
    ...
    private fun startParallelCourseTasks(courses: List<Course>, origin: String, label: String) {
        ...
        broadcastLog("启动并行选课：$label，共 $parallelTaskTotal 门，仅使用当前账号")
    ...
    private fun fillParallelTaskSlots(school: SchoolConfig) {
        ...
        updateNotification("并行选课中 | 运行：${activeGrabTasks.size} | 等待：${pendingGrabTasks.size} | 成功：$successCount | 失败：$failCount")
    ...
    private fun proceedParallelHealthCheck(school: SchoolConfig, task: GrabTaskContext) {
        checkServerHealth(school) { result ->
            ...
            updateNotification("并行选课中 | 成功：$successCount | 失败：$failCount | 尝试：$retryCount")
        }
    }
    ...
    private fun smartModeMatchFromAllCourses(school: SchoolConfig, task: GrabTaskContext) {
        ...
        CourseApiClient.getInstance().fetchAvailableCourses(school, buildCourseListBody(task.params, task.course), serviceAccountStorageKey, object : Callback {
            ...
            override fun onResponse(call: Call, response: Response) {
                ...
                try {
                    val courses = com.hnnujw.course.utils.CourseParser.parseCourseListFromJson(json)
    ...
    private fun verifyParallelSelectionAsync(school: SchoolConfig, task: GrabTaskContext) {
        Thread {
            ...
            handler.post {
                ...
                finishParallelTask(task, success = true, message = "选课成功")
            }
        }.start()
    }
    ...
    private fun finishParallelPoolIfIdle() {
        ...
        val summary = "并行选课完成。成功: $successCount/$parallelTaskTotal，失败: $failCount"
    ...
    private fun startGrabbing() {
        ...
        broadcastUpdate("开始选课：${course?.name}")
    ...
    private fun startNextQueueItem() {
        ...
        updateNotification("队列选课：${course.name} [${currentQueueIndex + 1}/${queue.size}]")
    ...
    private fun stopGrabbing() {
        ...
        val intent = Intent(BROADCAST_UPDATE).apply {
            ...
            putExtra(EXTRA_LOG_MESSAGE, "⏹ 选课已停止")
        ...
        if (!isRunning || retryCount >= maxRetry) {
            if (retryCount >= maxRetry) {
                ...
                if (keywordQueue.size > 1 && currentKeywordIndex < keywordQueue.size - 1) {
                    broadcastLog("当前关键词未选到，切换到下一个关键词…")
                    currentKeywordIndex++
                    isRunning = false
                    handler.postDelayed({
    ...
    private fun checkCookieValidity(school: SchoolConfig, callback: (Boolean) -> Unit) {
        ...
        CourseApiClient.getInstance().validateCookie(school, serviceAccountStorageKey, object : Callback {
            ...
            override fun onResponse(call: Call, response: Response) {
                ...
                // 判断逻辑：尝试解析学生姓名
                val studentName = com.hnnujw.course.utils.CourseParser.parseStudentName(html)
                ...
                handler.post {
                    ...
                    if (isValid) {
                        ...
                        if (com.hnnujw.course.utils.SessionRenewer.canRenew()) {
                            broadcastLog("注意：登录状态已失效，正在自动续期")
                            com.hnnujw.course.utils.SessionRenewer.request(expected) { result ->
                                if (result is com.hnnujw.course.utils.SessionRecoveryResult.Recovered && sessions.isCurrent(result.token)) {
                                    broadcastLog("成功：登录状态已恢复，继续执行")
                                    callback(true)
                                } else if (result is com.hnnujw.course.utils.SessionRecoveryResult.NeedsLogin && sessions.isCurrent(expected)) {
                                    handleCookieInvalid()
                                    callback(false)
                                }
                            }
    ...
    private fun handleCookieInvalid() {
        ...
        val notification = NotificationCompat.Builder(this, CHANNEL_ID)
            ...
            .setContentText("选课已停止，请点击此通知重新登录")
            .setSmallIcon(R.mipmap.ic_launcher)
                ...
                override fun onResponse(call: Call, response: Response) {
                    ...
                    if (details != null) {
                        ...
                        if (TeachingClassMatcher.canUseSavedClass(course) && !course.doJxbId.isNullOrEmpty() && !course.classId.isNullOrEmpty()) {
                            ...
                            broadcastLog("注意：解析失败，强制使用保存的 ID 选课")
...
private fun broadcastQueueUpdate(courseName: String? = null, status: String? = null, courseId: String? = null, queueStatusKey: String? = null) {
    ...
    private fun startKeywordGrabbing(keywords: String) {
        ...
        if (isParallelMode && keywordQueue.size > 1) {
            ...
            startParallelCourseTasks(keywordCourses, origin = "keyword", label = "关键词选课")
        } else {
            // 顺序模式：开始处理第一个关键词
            startSingleKeywordGrabbing(keywordQueue[currentKeywordIndex])
        }
    ...
    private fun startParallelGrabbing() {
        ...
        broadcastLog("并行启动 $workersToStart 个选课任务")
        ...
        try {
            ...
            if (tabParamsList.isEmpty()) {
                val defaultTab = TabParam(
                    ...
                    com.hnnujw.course.utils.CourseNameKit.resolveIndexXkkz(indexParams),
    ...
    private fun handleFallback(baseCourse: Course, queueCourse: Course?) {
        ...
        if (queueCourse != null && queueCourse.useExactMatch && !queueCourse.classId.isNullOrEmpty()) {
            broadcastLog("注意：使用队列中的精确配置进行强制选课：classId=${queueCourse.classId}")
    ...
    private fun useMatchedCourseAndGrab(match: Course) {
        ...
        // 保存 courseParams 供选课使用
        courseParams = indexParams.toMap()
        
        updateNotification("正在选课：${match.name}")
        startGrabbing()
    }
                ...
                try {
                    ...
                    for (i in 0 until classes.length()) {
                        ...
                        if (hasVacancy) {
                            ...
                            broadcastLog("人数变化，剩余 ${capacity - currentSelected} 个名额，立即选课")
                        }
                    }
                    ...
                    if (vacancyCourses.isNotEmpty()) {
                        ...
                        if (isParallelMode && vacancyCourses.size > 1) {
                            startParallelCourseTasks(vacancyCourses, origin = "fuzzy", label = "模糊候选选课")
                        } else {
    ...
    private fun executeFuzzyMatchSelection(school: SchoolConfig, course: Course) {
        broadcastLog("开始选课：${course.name} [${course.teacher}]")
        successCount = 0
        failCount = 0
        retryCount = 0
    ...
    private fun executeDirectSelection(school: SchoolConfig, course: Course) {
        ...
        retryCount++
        if (retryCount > 10) { // 模糊匹配选课最多尝试10次
            broadcastLog("注意：选课失败次数过多，恢复监控")
            isFuzzyMatchMode = true
            // 🔧 恢复监控时重新获取隐藏参数
            ...
            override fun onResponse(call: Call, response: Response) {
                ...
                try {
                    ...
                    if (success) {
                        ...
                        updateNotification("选课成功：${course.name}")
