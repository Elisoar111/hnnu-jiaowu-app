...
object AppearanceSettingsManager {
     ...
     * 全班同学。关掉之后仍会显示「我的排名」，只是不再铺开同学名单。
     */
    private const val KEY_SHOW_CLASS_RANK = "second_class_show_class_rank"
    ...
    /**
     * 是否显示第二课堂的班级同学排名（**默认开**，与历史行为一致）。
     *
     * 读的是 Compose state 而不是每次去查 SharedPreferences：二课页直接订阅它，
     * 拨动开关当帧生效，不需要走一次页面重建。
     */
    ...
    /** 「显示班级同学排名」开关。与玻璃开关同款：state 驱动，落盘持久化。 */
    fun updateShowClassRank(enabled: Boolean) {
    ...
    /**
     * 收下一张**已经过取景框裁剪**的图片当壁纸（见 `ImageCropActivity`）。
     *
     * 这是「自定义背景」现在的主路径：相册 → 取景框调整 → 存成壁纸。
     * 与 [importImageWallpaper] 的区别是不再自己解码相册 URI —— 裁剪结果已经
     * 是摆正、裁好、压过的一张本地文件，这里只接管"提主色 / 生成模糊层 / 落盘"。
     *
     * @param onResult 回主线程，false 表示这张图读不了（调用方去提示用户）。
     */
    fun importCroppedImageWallpaper(context: Context, cropped: File, onResult: (Boolean) -> Unit = {}) {
        val app = appContext ?: context.applicationContext.also { appContext = it }
        ioScope.launch {
            val imported = WallpaperImageStore.importCropped(app, cropped)
            val sharp = imported?.let { WallpaperImageStore.loadSharp(app)?.asImageBitmap() }
            val soft = imported?.let { WallpaperImageStore.loadSoft(app)?.asImageBitmap() }
            withContext(Dispatchers.Main) {
                if (imported == null || sharp == null) {
                    Log.w(TAG, "裁剪后的背景导入失败")
                    onResult(false)
                    return@withContext
                }
                imageColor = Color(imported.dominantColor)
    ...
    /** 切到已存在的图片壁纸。没有图片时什么都不做——否则背景会空掉。 */    fun useImageWallpaper() {
