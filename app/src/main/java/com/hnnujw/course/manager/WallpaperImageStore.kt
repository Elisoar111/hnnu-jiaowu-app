...
object WallpaperImageStore {
    ...
    /** 取景框裁剪结果的暂存名（cache 目录）。 */
    private const val STAGED_NAME = "wallpaper_crop_staged.jpg"
    ...
    /**
     * 取景框裁剪结果的暂存文件（cache，不是 filesDir）。
     *
     * 裁剪页把结果写到这里，调用方再拿它走 [importCropped] 生成正式的
     * sharp / soft 两层。放在 cache 是刻意的：中途取消/被杀留下的残片
     * 由系统回收，不会在用户目录里越积越多。
     */
    fun stagedFile(context: Context): File = File(context.cacheDir, STAGED_NAME)

    /**
     * 把一张**已经裁剪好**的图收成壁纸。
     *
     * 与 [import] 的区别只有一处：不再自己解码相册 URI、也不再自己摆正和裁剪 ——
     * 那些都由取景框裁剪页做完了（见 `ImageCropActivity`）。这里只负责
     * 生成模糊层、提主色、算色调图，然后落盘成正式壁纸。
     *
     * 这个入口是"背景也走取景框"的落点。旧路径直接把整张原图吃进来，
     * 用户构图完全不受控，竖屏手机上经常只看到原图中间一小条。
     *
     * @return 主色 / 尺寸 / 色调图；失败返回 null。
     */
    internal fun importCropped(context: Context, croppedFile: File): WallpaperImportResult? {
        if (!croppedFile.exists()) {
            Log.w(TAG, "裁剪结果不存在: ${croppedFile.absolutePath}")
            return null
        }
        var scaled: Bitmap? = null
        var soft: Bitmap? = null
        return try {
            val decoded = decodeFile(croppedFile) ?: run {
                Log.w(TAG, "裁剪结果解不出来")
                return null
            }
            val metrics = context.resources.displayMetrics
            // 裁剪结果已经是目标比例，这里只做"别超过屏幕长边"的收口
            val target = max(metrics.widthPixels, metrics.heightPixels).coerceIn(320, 4096)
            scaled = downscale(decoded, target)
            ...
            Log.w(TAG, "导入裁剪后的壁纸失败", t)
            clear(context)
            null
        } finally {
            listOf(soft, scaled).distinct().forEach { if (it != null && !it.isRecycled) it.recycle() }
            // 暂存文件用完即删，别在 cache 里留一份和正式壁纸同样大小的副本
