...
fun AcademicGradesRoute(school: SchoolConfig) {
    ...
    LaunchedEffect(account, revision, session.token) {
        ...
        if (revision == 0 && reportLoaded) {
            ...
            if (GradesCacheManager.isStale(cacheFetchedAt)) {
                try {
                    ...
                    com.hnnujw.course.academic.GradeWatcher.markSeen(context, account)
        ...
        try {
            ...
            if ((e as? AcademicException)?.status == AcademicStatus.SESSION_EXPIRED)
                com.hnnujw.course.network.CourseApiClient.getInstance().notifyCookieExpired(expected)
        }
    ...
    com.hnnujw.course.ui.system.ReportPageContent(report.grades.isNotEmpty() || exams.isNotEmpty())
