...
fun SelectedCoursesRoute(refreshRevision: Int = 0, onLoadingChange: (Boolean) -> Unit = {}) {
    ...
    val requests = remember(sessions) { com.hnnujw.course.manager.SessionRequestGate(sessions) }
    DisposableEffect(requests) { onDispose { requests.cancelAll() } }
    
    var courses by com.hnnujw.course.ui.system.rememberPageData<List<Course>>("legacy.selected") { emptyList() }
    var loadedRevision by com.hnnujw.course.ui.system.rememberPageData("legacy.selected.revision") { -1 }
    var isLoading by remember { mutableStateOf(false) }
    var isDropping by remember { mutableStateOf(false) }
    
    // 缓存从服务器获取的学年/学期参数（退课 API 需要）
    var xkxnm by com.hnnujw.course.ui.system.rememberPageData("legacy.selected.year") { "" }
    var xkxqm by com.hnnujw.course.ui.system.rememberPageData("legacy.selected.term") { "" }

    fun isCurrentAccount(ticket: com.hnnujw.course.manager.SessionRequestTicket): Boolean {
