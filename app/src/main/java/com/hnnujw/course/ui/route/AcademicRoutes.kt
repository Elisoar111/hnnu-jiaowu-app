...
import com.hnnujw.course.ui.theme.ModuleMotion
...
import com.hnnujw.course.service.GrabService
import com.hnnujw.course.ui.screen.CourseListScreen
import com.hnnujw.course.ui.screen.GrabProScreen
import com.hnnujw.course.ui.screen.SelectedCoursesScreen
...
fun AcademicCourseListRoute(school: SchoolConfig) {
    ...
    // 区分"学校没开放选课"和"真的加载失败"：前者不该报成故障
    var errorStatus by remember(account) { mutableStateOf<AcademicStatus?>(null) }
    // 学校在选课首页写的提示语（如"当前不属于选课阶段"），列表为空时用来解释原因
    var roundNotice by remember(account) { mutableStateOf("") }
    ...
    LaunchedEffect(account, selectedCategory, revision, expectedSession) {
        ...
        try {
            ...
            if (sessions.isCurrent(expectedSession)) {
                ...
                roundNotice = page.context.notice
                coursesLoaded = true
            }
        } catch (e: CancellationException) { throw e }
        catch (e: Exception) {
            if (sessions.isCurrent(expectedSession)) {
                error = e.message ?: "课程加载失败，请重试"
                errorStatus = (e as? AcademicException)?.status
            }
        }
    ...
    fun add(course: Course) {
        if (AcademicGrabRuntimeStore.get(account).running) { GlassToaster.show("请先停止当前选课任务再修改队列"); return }
        ...
        GlassToaster.show(if (added) "已加入选课队列" else "课程已在队列中")
    }

    fun setTarget(course: Course, exact: Boolean) {
        if (AcademicGrabRuntimeStore.get(account).running) { GlassToaster.show("请先停止当前选课任务再更换目标"); return }
    ...
    Scaffold(containerColor = Color.Transparent, topBar = {
        Column(Modifier.fillMaxWidth().moduleEntrance(0).reportNoticeAnchor()) {
            ...
            if (tab == 0) {
                ...
                if (categories.isNotEmpty()) SystemPicker(
                    options = listOf("全部轮次与分类") + categories.map { it.name },
        ...
        Column(Modifier.fillMaxSize().padding(top = padding.calculateTopPadding()).moduleEntrance(1)) {
            if (tab == 1) {
                ...
                if (error.isNotBlank()) {
                    // 轮次没开放不是故障，别用"加载失败"吓人（淮南师范学院未开放时返回
                    // "对不起，当前不属于选课阶段！"，走到这里应显示成正常状态）
                    val roundClosed = errorStatus == AcademicStatus.ROUND_CLOSED
                    Box(Modifier.fillMaxSize().padding(24.dp), contentAlignment = Alignment.Center) {
                        SystemEmptyState(
                            title = if (roundClosed) "当前暂无选课轮次" else "课程加载失败",
                            message = if (roundClosed) "${error.trimEnd('！', '!', '。')}。已选课程、课表和成绩仍可正常查询。"
                            else error
                        ) {
                            SystemSecondaryButton(
                                text = if (roundClosed) "刷新轮次" else "重新加载",
                                onClick = { revision++ }
                            )
                        }
                    }
                } else if (!loading && courses.isEmpty() && roundNotice.isNotBlank()) {
                    // 学校未开放轮次时列表接口回的是空数组而不是错误，光看结果分不清原因，
                    // 这里直接把学校页面上的原话摆出来（淮南师范学院："当前不属于选课阶段"）。
                    Box(Modifier.fillMaxSize().padding(24.dp), contentAlignment = Alignment.Center) {
                        SystemEmptyState(
                            title = "当前暂无选课轮次",
                            message = "$roundNotice 已选课程、课表和成绩仍可正常查询。"
                        ) {
    ...
    if (showFilters) SystemDialog(onDismissRequest = { showFilters = false }, title = { Text("课程筛选") },
        ...
        Column(Modifier.fillMaxWidth().heightIn(max = 420.dp).verticalScroll(rememberScrollState()), verticalArrangement = Arrangement.spacedBy(12.dp)) {
            com.hnnujw.course.ui.screen.SchoolFormField("教师", draftFilter.teacher, { draftFilter = draftFilter.copy(teacher = it) })
            com.hnnujw.course.ui.screen.SchoolFormField("上课时间", draftFilter.time, { draftFilter = draftFilter.copy(time = it) })
            com.hnnujw.course.ui.screen.SchoolFormField("上课地点", draftFilter.location, { draftFilter = draftFilter.copy(location = it) })
            com.hnnujw.course.ui.screen.SchoolFormField("学分", draftFilter.credit, { draftFilter = draftFilter.copy(credit = it) })
...
fun AcademicGrabQueueRoute(school: SchoolConfig) {
    ...
    fun start(scheduled: Boolean = false) {
        ...
        if (scheduled) {
            val trigger = com.hnnujw.course.utils.GrabTaskUtils.parseScheduledDateTime(scheduledDateTime)
    ...
    val notificationPermission = rememberLauncherForActivityResult(ActivityResultContracts.RequestPermission()) { granted ->
        if (granted) start(pendingScheduledStart) else GlassToaster.show("需要通知权限以显示选课状态")
    }
    ...
    GrabProScreen(isRunning = running, successCount = success, failCount = failed, retryCount = retries,
        ...
        queueItemStatuses = itemStatuses.mapValues { (_, value) ->
            if (!running && value == "GRABBING") com.hnnujw.course.ui.screen.GrabQueueItemStatus.WAITING
            else runCatching { com.hnnujw.course.ui.screen.GrabQueueItemStatus.valueOf(value) }.getOrDefault(com.hnnujw.course.ui.screen.GrabQueueItemStatus.WAITING)
        },
    ...
    if (showDateTimePicker) GlassDateTimePickerDialog(title = "选择选课时间",
        initialMillis = remember(scheduledDateTime) {
            com.hnnujw.course.utils.GrabTaskUtils.parseScheduledDateTime(scheduledDateTime)
                ?: System.currentTimeMillis()
        }, onConfirm = { millis ->
        scheduledDateTime = com.hnnujw.course.utils.GrabTaskUtils.formatScheduledDateTime(millis)
    ...
    if (showManualAdd) SystemDialog(onDismissRequest = { showManualAdd = false }, title = { Text("添加课程到队列") },
        content = {
            Column(Modifier.verticalScroll(rememberScrollState()), verticalArrangement = Arrangement.spacedBy(12.dp)) {
                com.hnnujw.course.ui.screen.SchoolFormField(label = "课程名称", value = inputCourse, onValueChange = { inputCourse = it }, placeholder = "完整课程名称")
                if (school.academicSystem == AcademicSystem.ZF.id) {
                    com.hnnujw.course.ui.screen.SchoolFormField(label = "教学班（选填）", value = inputSection, onValueChange = { inputSection = it },
                        placeholder = "例如：篮球0003", helper = "按教学班名称匹配，留空则不限")
                }
                com.hnnujw.course.ui.screen.SchoolFormField(label = "教师（选填）", value = inputTeacher, onValueChange = { inputTeacher = it })
                com.hnnujw.course.ui.screen.SchoolFormField(label = "上课时间（选填）", value = inputTime, onValueChange = { inputTime = it })
            }
        }, confirmButton = {
