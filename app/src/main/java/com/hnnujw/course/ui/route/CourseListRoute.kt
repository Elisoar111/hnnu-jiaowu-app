...
import com.hnnujw.course.manager.CourseCacheManager
...
import com.hnnujw.course.ui.screen.toDynamicDisplayTags
import com.hnnujw.course.ui.route.SelectedCoursesRoute
...
private data class CourseListRouteSnapshot(
    ...
    val activeFilter: com.hnnujw.course.model.CourseFilter?,
    val draftFilter: com.hnnujw.course.model.CourseFilter,
...
fun CourseListRoute() {
    ...
    if (!UserManager.getInstance().isDemoMode && academicSchool != null && com.hnnujw.course.academic.AcademicGatewayFactory.supports(academicSchool)) {
    ...
    val requests = remember { com.hnnujw.course.manager.SessionRequestGate(UserManager.getInstance().sessionState) }
    ...
    var draftFilter by remember(routeAccountKey) {
        mutableStateOf(restoredSnapshot?.draftFilter ?: com.hnnujw.course.model.CourseFilter())
    }
    ...
    // 筛选应用回调
    val onFilterApply: (com.hnnujw.course.model.CourseFilter) -> Unit = { filter ->
        activeFilter = filter
        showFilterPanel = false
        if (filter.isEmpty()) {
            courses = allCourses
    ...
    val onFilterClear: () -> Unit = {
        activeFilter = null
        draftFilter = com.hnnujw.course.model.CourseFilter()
        courses = allCourses
    }
    ...
    val performSelection = remember {
        fun(course: Course) {
            if (isDemoMode) {
                ...
                GlassToaster.show("演示选课成功：${course.name}")
                return
            }
            val userManager = UserManager.getInstance()
            val school = userManager.currentSchool ?: return
    ...
    val performBatchSelection = remember {
        fun(selectedCourses: List<Course>) {
            if (isDemoMode) {
                ...
                courses = courses.toList()
                allCourses = allCourses.toList()
                GlassToaster.show("演示批量选课完成：成功 ${selectedCourses.size} 门")
                return
            }
            val userManager = UserManager.getInstance()
            val school = userManager.currentSchool ?: return
            ...
            GlassToaster.show("开始批量选课，共 ${selectedCourses.size} 门课程")
            ...
            scope.launch(Dispatchers.IO) {
                ...
                selectedCourses.forEachIndexed { index, course ->
                    ...
                    withContext(Dispatchers.Main) {
                        if (!isCurrentAccount(requestAccountKey)) return@withContext
                        GlassToaster.show("正在选课 (${index + 1}/${selectedCourses.size}): ${course.name}")
                    }
                ...
                withContext(Dispatchers.Main) {
                    if (!isCurrentAccount(requestAccountKey)) return@withContext
                    isBatchSelecting = false
                    GlassToaster.show("批量选课完成！成功：$successCount 门，失败：$failCount 门")
                }
            }
        }
    }
    ...
    // 🔧 视图切换容器（带平滑动画）
    val topBarBackdrop = com.hnnujw.course.ui.system.LocalAppBackdrop.current
    val topBarUseGlass = topBarBackdrop != null && com.hnnujw.course.ui.system.isBackdropSupported()
    ...
    val screen = com.hnnujw.course.ui.system.rememberScreenMetrics()
    val topBarHeight = com.hnnujw.course.ui.system.TopBarLayoutMetrics.height()
    val topBarSegmentHeight = com.hnnujw.course.ui.system.TopBarLayoutMetrics.segmentHeight()
    ...
    val topBarSegmentWidth = com.hnnujw.course.ui.system.TopBarLayoutMetrics.segmentWidth()
    val topBarTint = if (com.hnnujw.course.ui.system.rememberGlassDarkTheme()) MaterialTheme.colorScheme.surface.copy(alpha = 0.9f) else White.copy(alpha = 0.60f)
    com.hnnujw.course.ui.theme.ReportStatusBarSurface(topBarTint)
    Scaffold(
        ...
        topBar = {
            ...
            Box(modifier = topBarShellModifier) {
                    ...
                    when (mode) {
                        "search" -> {
                            ...
                            Row(
                                ...
                                com.hnnujw.course.ui.system.GlassTextField(
                                    value = searchQuery,
                            ...
                            CenterAlignedTopAppBar(
                                ...
                                title = {
                                    ...
                                    Box(modifier = Modifier.padding(end = SegmentedPressSlack)) {
                                        com.hnnujw.course.ui.system.SystemSegmentedControl(
                                ...
                                actions = {
                                    ...
                                    com.hnnujw.course.ui.system.TopBarActionRail(
                                        spacing = 4.dp,
                                        ...
                                        action(
                                            ...
                                            com.hnnujw.course.ui.system.AnimatedLineIcon(
                                                com.hnnujw.course.ui.system.AnimatedIconSpec.Refresh,
                                                Modifier.size(com.hnnujw.course.ui.system.TopBarLayoutMetrics.IconSize),
                                                state = if (busy) com.hnnujw.course.ui.system.IconVisualState.Running
                                                    else com.hnnujw.course.ui.system.IconVisualState.Idle)
                                        }
                ...
                // 底部柔和渐变线（仅回退路径显示）
                if (!topBarUseGlass) {
                    com.hnnujw.course.ui.system.SystemDivider(alpha = 0.6f)
                }
                }
            }
            }
        ...
        Box(modifier = Modifier.fillMaxSize().padding(paddingValues).moduleEntrance(1)) {
                    ...
                    CourseListScreen(
                        ...
                        onAddToQueue = { course ->
                            if (isDemoMode) {
                                ...
                                GlassToaster.show(if (added) "已加入演示选课队列：${course.name}" else "已经在演示队列中：${course.name}")
                            } else {
                                ...
                                GlassToaster.show("已加入选课队列：${course.name}")
                            } else {
                    ...
                    dismissButton = {
                        SystemSecondaryButton(
                            ...
                            onClick = {
                                ...
                                GlassToaster.show(if (added) "已加入选课队列：${course.name}" else "已经在队列中：${course.name}")
                            },
                            modifier = Modifier.fillMaxWidth()
                        )
                    }
            ...
            androidx.compose.animation.AnimatedVisibility(
                ...
                modifier = Modifier
                    ...
                    .padding(bottom = com.hnnujw.course.ui.system.LocalAppOverlayBottomInset.current),
                enter = slideInVertically { it * 2 } + fadeIn(),
                exit = slideOutVertically { it * 2 } + fadeOut()
            ) {
                com.hnnujw.course.ui.system.LiquidButton(
                    onClick = {
                    ...
                    style = com.hnnujw.course.ui.system.LiquidButtonStyle.SolidTinted,
                    ...
                    Text(
                        text = "批量选课 (${selectedClassIds.size})",
...
internal fun FilterActionContent(activeCount: Int, expanded: Boolean, onAnchor: (androidx.compose.ui.geometry.Offset) -> Unit) {
    ...
    Box(modifier = Modifier.size(com.hnnujw.course.ui.system.TopBarLayoutMetrics.TouchTarget)
        .onGloballyPositioned { onAnchor(it.boundsInWindow().center) }, contentAlignment = Alignment.Center) {
        com.hnnujw.course.ui.system.AnimatedLineIcon(
            spec = com.hnnujw.course.ui.system.AnimatedIconSpec.Filter,
            state = if (expanded) com.hnnujw.course.ui.system.IconVisualState.Expanded else com.hnnujw.course.ui.system.IconVisualState.Idle,
            modifier = Modifier.size(com.hnnujw.course.ui.system.TopBarLayoutMetrics.IconSize),
            tint = if (active) NeuPrimary else androidx.compose.material3.LocalContentColor.current
        )
        com.hnnujw.course.ui.system.FilterCountBadge(activeCount,
...
private class CourseListLogicHelper(
    ...
    val requestSession: com.hnnujw.course.manager.SessionToken,
