...
import com.hnnujw.course.ui.screen.LoginScreen
...
import com.hnnujw.course.login.PasswordLoginGateway
import com.hnnujw.course.login.PasswordLoginGatewayFactory
import com.hnnujw.course.academic.AcademicGatewayFactory
import com.hnnujw.course.academic.AcademicPasswordLoginGateway
import com.hnnujw.course.AcademicWebViewActivity
...
class LoginActivity : ComponentActivity() {
    ...
    private val webViewLauncher = registerForActivityResult(
        ...
        if (result.resultCode == RESULT_OK) {
            val cookie = result.data?.getStringExtra(AcademicWebViewActivity.EXTRA_COOKIE_RESULT)
        ...
        lifecycleScope.launch {
            if (!BuildConfig.UI_PREVIEW) try {
                withContext(Dispatchers.IO) {
                    com.hnnujw.course.activation.ActivationManager.checkActivation(this@LoginActivity)
                }
                Log.d(TAG, "已同步云端配置")
            } catch (e: Exception) {
            ...
            CourseSelectorTheme {
                ...
                LoginScreen(
                    ...
                    onConfirmBinding = {
                        ...
                        val recorded = com.hnnujw.course.manager.StudentLimitManager.recordStudent(
                        ...
                        if (recorded) {
                            ...
                            discardPendingPasswordLogin()
                            errorMessage = "该设备最多绑定 ${com.hnnujw.course.manager.StudentLimitManager.MAX_STUDENTS} 个学生账号"
                        }
                    },
                    onCancelBinding = {
                        showBindingDialog = false
    ...
    private fun handleLogin(cookieStr: String) {
        ...
        lifecycleScope.launch {
            try {
                ...
                Log.d(TAG, "激活配置已同步，max_students=${com.hnnujw.course.activation.ActivationManager.getMaxStudents(this@LoginActivity)}")
            } catch (e: Exception) {
        ...
        if (AcademicGatewayFactory.supports(currentSchool)) {
            ...
            validationJob = lifecycleScope.launch {
                ...
                result.onSuccess { identity ->
                    if (identity.status != com.hnnujw.course.academic.AcademicStatus.SUCCESS) {
                        isLoading = false
            ...
            override fun onResponse(call: Call, response: Response) {
                ...
                runOnUiThread {
                    ...
                    if (success) {
                        ...
                        val maxStudents = com.hnnujw.course.activation.ActivationManager.getMaxStudents(this@LoginActivity)
                        val bindingCheck = com.hnnujw.course.manager.StudentLimitManager.checkCanUseStudent(
                            context = this@LoginActivity,
    ...
    private fun finishAcademicLogin(currentSchool: SchoolConfig, cookieStr: String, parsedName: String, parsedId: String) {
        ...
        runOnUiThread {
            ...
            if (bindingCheck.alreadyBound) {
                ...
                bindingMaxStudents = com.hnnujw.course.activation.ActivationManager.getMaxStudents(this)
