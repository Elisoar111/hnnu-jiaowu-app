 ...
 * 正方教务（jwglxt V9）适配器。
 *
 * 本应用只服务淮南师范学院：该校使用直登表单 + RSA 密码加密，没有统一认证入口，
 * 因此这里只实现直登表单登录。
 */
internal class ZfAcademicAdapter(school: SchoolConfig, session: AcademicSession, transport: AcademicHttpTransport) :
    ...
    // 直登表单在需要人机验证时会要求走 WebView，不存在应用内验证码会话。
    override suspend fun submitCaptcha(code: String): LoginResult = serial {
        LoginResult(AcademicStatus.SESSION_EXPIRED, message = "登录会话已失效，请重新登录")
    }

    override suspend fun refreshCaptcha(): CaptchaChallenge? = serial { null }
    ...
    override suspend fun loadCourseContext(): CourseContext = serial {
        ...
        // 正方 v5 变体入口页没有 queryCourse 调用：首个选课分类放在 first* 隐藏域，
        // 由页面脚本拷贝到 kklxdm/xkkz_id 等字段后发请求。不读它们会让分类参数全空。
        ...
        // 入口页一个分类都没有时，读学校自己写的提示。未开放轮次时正方返回的是
        // "对不起，当前不属于选课阶段…"，而列表接口只会回一个空数组；
        // 不把这句话带出去，界面上就分不清"没有可选课程"和"学校没开放轮次"。
        val notice = if (categoryRows.isEmpty() && AcademicJson.status(indexResponse.text) == AcademicStatus.ROUND_CLOSED)
            AcademicHtml.pageNotice(indexResponse.text).ifBlank { "学校当前未开放选课" } else ""
