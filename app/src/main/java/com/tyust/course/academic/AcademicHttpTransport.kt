...
class AcademicHttpTransport(
    ...
    private fun ensureAllowed(url: HttpUrl) {
        // 用「当前实际使用」的协议校验：备用站若为 HTTP，主协议是 HTTPS 会被误判。
        // 这里判定的目的是防意外跨站，不是防用户主动选的备用站。
        val activeProtocol = school.getActiveProtocol()
        val allowed = AcademicUrlPolicy.isAllowed(url.toString(), activeProtocol, allowedHosts)
        if (!allowed) throw AcademicException(AcademicStatus.UNTRUSTED_URL, "请求被拒绝：目标地址不在学校允许范围内（${url.host}）")
        if (activeProtocol.equals("https", true) && url.isHttps.not()) {
            throw AcademicException(AcademicStatus.UNTRUSTED_URL, "当前配置为 HTTPS，不能降级到 HTTP")
        }
    }
