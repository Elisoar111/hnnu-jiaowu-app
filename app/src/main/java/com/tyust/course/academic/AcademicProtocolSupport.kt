...
internal object AcademicJson {
    ...
    /**
     * 响应里找不到列表数组时抛出的异常。
     * 轮次未开放时学校返回的是提示页（HTML）或提示 JSON，而不是列表：
     * 这时把学校原文带出去，否则上层只能报"无法识别的列表"，用户会以为适配坏了。
     */
    private fun unresolvedList(body: String): AcademicException {
        ...
        val jsonMessage = if (trimmed.startsWith("{") || trimmed.startsWith("[")) message(body) else ""
        val detail = if (resolved == AcademicStatus.ROUND_CLOSED)
            AcademicHtml.pageNotice(body).ifBlank { jsonMessage }.ifBlank { "学校当前未开放选课" }.take(100)
        else "教务返回了无法识别的列表，请重新进入页面"
        return AcademicException(resolved, detail)
    }
    ...
    fun status(body: String, code: Int = 200): AcademicStatus {
        ...
        // 各校对"轮次没开"的说法差别很大：正方常见"当前不属于选课阶段"（淮南师范学院），
        // 也有"不在选课阶段"、"非选课阶段"。漏判会让提示页被当成"列表无法识别"，
        // 用户看到的是适配坏了，其实是学校没开放。
        if (listOf("未开始", "未开放", "已结束", "选课时间已过", "不在选课时间", "只可退课",
                "不属于选课阶段", "不在选课阶段", "非选课阶段", "选课阶段已结束", "选课已结束")
                .any(detail::contains)) return AcademicStatus.ROUND_CLOSED
