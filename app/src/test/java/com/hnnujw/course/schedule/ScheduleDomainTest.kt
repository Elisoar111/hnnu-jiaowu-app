...
class ScheduleDomainTest {
    ...
    @Test fun calendarWeekUsesLocalCivilDatesAcrossDaylightSavingBoundaries() {
        val zone = java.util.TimeZone.getTimeZone("Europe/Berlin")
        val later = com.hnnujw.course.schedule.ScheduleDates.date("2026-10-19", 2, zone = zone)!!
    ...
    @Test fun weekdayDatesRequireConfiguredTermStartAndFollowItOnceSet() {
        val zone = java.util.TimeZone.getTimeZone("UTC")
        // 未配置学期起始日时 semesterStartDate=0，dateFromMillis 返回空串。
        // 这是星期下的日期整行取不到值（用户看到"完全空白"）的根因，锁住它。
        assertNull(ScheduleDates.firstMonday(""))
        assertNull(ScheduleDates.date("", 3, 1, zone))
        assertNull(ScheduleDates.date(null, 3, 1, zone))
        // 配成周一之后，七个日列应给出连续日期（2026-09-07 是周一）。
        val start = "2026-09-07"
        val days = (1..7).map {
            ScheduleDates.date(start, 1, it, zone)!!.get(java.util.Calendar.DAY_OF_MONTH)
        }
        assertEquals(listOf(7, 8, 9, 10, 11, 12, 13), days)
        // 第二周整体后移 7 天。
        assertEquals(14, ScheduleDates.date(start, 2, 1, zone)!!.get(java.util.Calendar.DAY_OF_MONTH))
    }
}
