...
- 顺序：① 拿到安装包 → ② 卸载手机上的旧版本 → ③ 安装下载好的包
- 应用内的「更新」按钮照常能用：下载完它会额外把安装包留一份到手机自带的「下载」目录（Android 10 及以上），卸载旧版不会把这份删掉 —— 卸载完打开「文件管理 → 下载」，点 `hnnu-jiaowu-v1.2.6.apk` 安装即可
- 若点「更新」后系统提示「应用未安装 / 安装包与已安装应用签名不一致」，正是这个冲突：按上一条从「下载」目录里装，不必重新下载
- Android 9 及更早的系统请改用浏览器到发布页下载 `app-release.apk`：这些系统上应用没有写「下载」目录的权限，内置下载的安装包会随卸载一起消失
- 为什么要换：此前线上包一直用开发调试密钥签名，而调试密钥的口令与别名是公开约定（任何一台开发机都有同款）。换成只有维护者持有的正式密钥之后，别人就没法再签出一个"同名"的安装包来顶替你手机上的应用
- 代价是这一次要从头来一遍：卸载会一并清掉本机保存的登录状态，以及课表、成绩、消息这些本地缓存
- 纯本地的设置也要重做：自定义课程、组件工作台的卡片排版、课表显示偏好、主题与字体
- 课表与成绩重新登录后会自动同步回来，这部分不用手动恢复
- 只影响这一次：从此往后的版本之间都能正常覆盖升级，不用再卸载

### 组件工作台：课表、考试、成绩摆成一页
课表右上角「··· → 组件工作台」是一块能自己排布的信息页，七种卡片长按即可拖动换位、逐张改尺寸，点哪一块就跳到对应的页面。

- 七种卡片随你挑：下一节课、今日课程、今日时间轴、考试倒计时、成绩概览、二课积分、消息未读
- 每种卡片都能在「小 / 宽 / 中 / 大」四档尺寸里切换，宽的铺满一行，中的占半页
- 长按任意卡片进入编辑态，拖动到想要的位置松手即可；编辑态下卡片右上角出现改尺寸与移除按钮
- 加卡片时先在组件抽屉里看实时预览，确认是想要的样子再放上去
- 点击跳转：课表类卡片回到本周今天，考试倒计时跳到考试页，成绩概览跳到成绩，二课积分跳到第二课堂，消息未读直接打开消息中心
- **一行一门课**：「今日课表」的「下一节」行、「今日时间轴」的每一行都能单独点开，直接跳到课表并打开那门课的详情 —— 应用内与桌面卡片都一样
- 排版是这台设备上的个人偏好，卡片里的内容则严格跟着你当前登录的账号走：换账号后布局保持你摆好的样子，内容自动切换
- 卡片读的都是本地已有数据，不联网、不额外消耗流量

### 同一批卡片可以直接放到手机桌面
桌面（壁纸上面那一层）也能摆这七种卡片，内容与应用里那张一模一样，点一下直接跳过去。

- 从桌面加（推荐，真机实测最靠得住的一条）：长按桌面空白处 →「卡片」→ 卡片中心 → 一直往下滑到最底部 →「插件」→ 选「校园助理」→ 点要加的卡片。ColorOS（OPPO / 一加 / realme）走的就是这条，它的长按菜单里**没有**「小组件」这一项，找不到是正常的
- 从工作台加：长按卡片进入编辑态，点右上角的「添加到桌面」，或在组件抽屉里直接选「添加到桌面」。多数系统会弹确认框、点「添加」即可；ColorOS 上这个框会自己关掉，此时请用上面那条桌面路径
- **拉大之后会多出内容**：组件按桌面上给它的实际格子出图，拉宽拉高会长出教室、更多课程行和当日汇总，不是把小尺寸的样子硬拉大
- 系统字体调大后，卡片会相应少画几行而不是把字裁一半；浅色 / 深色主题各自一套底色与文字色
- 组件抽屉里会额外给一张「桌面上最小尺寸的样子」—— 与桌面组件用的是同一套渲染，看到的就是加上去的
- 原来的三种课表小组件已下线：它们只认课表，与现在这七种卡片重复。桌面上还摆着旧组件的会被系统自动移除，请按上面的方式重新添加

### 一卡通：余额、消费与宿舍电费，登录一次全都能看
「我的 → 校园服务 → 一卡通」新增校园卡查询：卡内余额、账户总额、宿舍电费与按月消费汇总，全部只读。

- 账户总额按学校的口径合计，与校园卡官方页面一致；同一张卡里直接列出卡号、账号、有效期与电费账户，不用在两处之间对照
- 宿舍电费按房间计量，要先依次选**校区 → 楼栋 → 楼层 → 房间**；选一次就记住了，下次进来直接出数
- 学校有两套电费入口（「1-6单元」与「7-11单元及东区」），按你住的楼栋选对那一套
- 有的费项返回的是**电量（度）**而不是金额，界面会标出来，并且不会拿它去套「余额不足」的提醒
- 电费低于 20 元就会提醒（阈值可改），**不限提醒次数** —— 后台每日巡检一次，所以最坏情况是每天一条，不会刷屏
- 消费按月看支出与收入两行，左右箭头切换本月 / 上月 / 更早；需要逐笔明细时一键跳学校的官方账单页
- 挂失 / 解挂校园卡是唯一的写操作：按钮点下去不会立即执行，要先看完后果说明，再手打一句确认语并输入查询密码
- 校园卡登录用的是学校的统一身份认证（和教务一样在官方页面上完成），本应用**不保存你的一卡通密码**，也**不提供充值、缴费** —— 需要充值时请走学校官方渠道

### 空闲教室：按周次、星期、节次找没课的教室
「我的 → 校园服务 → 空闲教室」新增空闲场地查询，选好校区、周次、星期和节次，就能列出这些时段空着的教室。

- 筛选条件与教务网页一致：校区（必选）、楼号、场地类别、座位数区间、场地名称关键词，后面几项不填表示不限
- 周次和节次都可以**多选**，多选时按「这些时段全都空着」来筛
- 换校区后楼号与节次的候选会跟着换，已选的楼号与节次会被清空 —— 不会拿 A 校区的楼号去查 B 校区
- 一次取 100 条，标题右侧给出命中总数，末尾有「加载更多」继续翻页
- 只做查询：网页上的「场地借用申请」是向学校提交写请求，本应用不代提交

### 课程提醒：总开关、提前时间与上课自动勿扰
「课表设置 → 课程提醒」把提醒设置收成一页：总开关、提前多久提醒、上课时自动切勿扰，还有一份权限自检。

- 提前时间可选「上课时」与提前 5 / 10 / 15 / 20 / 30 / 45 / 60 分钟，默认 15 分钟；改完已设的提醒会立即按新时间重排
- **上课自动模式**：上课时自动把手机切到「勿扰 / 仅闹钟 / 仅优先」，下课时还原成你原来的档位
- 自动模式**只在你授权了勿扰访问权限时才会生效** —— 没有权限时应用根本不去动系统设置，因为改了就可能还原不了
- 页内自带权限自检（通知、精确闹钟、勿扰、后台自启、忽略电池优化），每一项都能直接点开对应的系统设置页
- 总开关关闭后不再排任何提醒，逐门课程里的开关也一并失效，课程详情页会写明原因

### 课表：日页日期条跟着翻页，还能只看工作日
课表这版有三处改进：日视图的日期条会跟着翻页走，没有周末课时可以把周末两列收起来，补课的目标周与目标天都能自己挑。

- 日视图左右滑课表时，顶上那条日期会有一个滑块跟着走 —— 翻到哪天，哪一格就被圈出来；今天那一格底下另有一个小圆点，翻到第几周都找得到今天
- 点日期条上的任意一天可以直接跳过去；周视图里点某天会切到日视图并落到那一天
- 周视图里这个滑块只用来标「今天」：翻到别的周时它不出现，而不是随便停在第 1 格
- 没有周末课时，可以把周末那两列收起来 —— 五列比七列宽约四成，课程名称和上课地点都更清楚。入口：课表「··· → 课表设置 → 课表显示 → 显示周末」
- 日视图不受影响，仍是完整的七天；万一这周确实有周末的课被藏起来，网格下方会写一行「另有 N 门周末课程，可在日视图查看」，不会让你以为课丢了
- 补课弹窗里「补到第几周 / 补到哪一天 / 要补哪一周 / 要补哪一天」四个都能改，补完若落到了别的周，课表会直接跳到那一格

### 检查更新更及时
修掉了「检查更新」的一个老毛病：以前只要有一次没连上网，接下来六小时都不会再自动检查，装上新版本也一直等不到提示。

- 只有真正检查成功才会记时间，一次失败不再让后续检查被静默跳过
- 断网、服务异常时照常在下一次启动时重试
...
- 入口：设置 → 外观 → 底部导航栏自动收起
- 关掉之后底栏立即展开并保持展开，重启后依然是关的
- 重新打开时不会沿用关掉之前的滚动手势，不会一开就自己收回去

### 选课功能已移除，学工系统顶到底栏
底栏课表旁边那个位置原来放的是「选课」，现在换成了「学工系统」；选课功能在本版整体下线，需要选课请使用学校教务网页。

- 学工系统升为常驻页面：日常请假、节假日去向登记，底栏点一下就到，不用再进设置里翻
- 「我的 → 校园服务」现在是「考级项目报名」「空闲教室」「一卡通」三项
- 「启动页」选项里的「选课」也随之换成「学工」；旧版本里把启动页设成选课的，升级后会回退到课表


## 结尾
感谢每一位使用与支持本项目的同学。

<!-- 需要「不升级就没法用」的版本时，取消下面这一节的注释（标记行会进 Release 正文，
     下一行用户看得到，所以必须写成人话）。平时保持注释状态：

## 强制更新
本次更新是必须的：旧版已无法继续同步课表与成绩，请更新后再使用。
-->
